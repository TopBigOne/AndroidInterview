...
void printJavaStack() {
    ...
    jstring java_stack = static_cast<jstring>(jniEnv->CallStaticObjectMethod(kJavaClass,
                                                                             kMethodGetStack));
...
Java_com_jar_plt_1hook_ThreadHook_enableThreadHookNative(JNIEnv *env, jclass clazz) {
    enableThreadHook();
}
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(
            env->FindClass("com/dodola/thread/ThreadHook")));
    if (kJavaClass == nullptr) {
        ALOG("InitJniEnv kJavaClass NULL");
        return false;
    }

    kMethodGetStack = env->GetStaticMethodID(kJavaClass, "getStack", "()Ljava/lang/String;");
    if (kMethodGetStack == nullptr) {
