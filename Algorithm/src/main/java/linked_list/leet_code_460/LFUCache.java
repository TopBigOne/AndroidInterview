...
package linked_list.leet_code_460;
 ...
 * @author : dev
 * @version :
 * @Date : 2/20/21 23:48 PM
 * @Desc : LRU 缓存-最不经常使用（https://leetcode-cn.com/problems/lfu-cache/）
...
public class LFUCache {
    ...
    public void put(int key, int value) {
        ...
        if (node != null) {
            ...
            // 若key不存在
            if (size == capacity) {
                // 如果缓存满了，删除lastLinkedList.pre这个链表（即表示最小频次的链表）中的tail.pre这个Node（即最小频次链表中最先访问的Node），
                // 如果该链表中的元素删空了，则删掉该链表。
